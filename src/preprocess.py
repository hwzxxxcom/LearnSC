...
class SampleSubgraph:
    ...
    def find_subgraph_reduced(self, candidate_info, query, graph):
        t_0 = time.time()
        candidates = [list(map(int, candidate_info[i * 2 + 1].strip().split())) for i in range(len(candidate_info) // 2)]
        all_candidates = list()
        assert max(self.query[0]) == len(self.query[0]) - 1
        for i in range(len(candidates)):
            for j in range(len(candidates[i])):
                all_candidates.append(candidates[i][j])
        all_candidates = list(set(all_candidates))
        ...
        for u in sorted(list(query.nodes)):
            for v in query.neighbors(u):
                if v <= u: continue
                for cand_u in candidates[u]:
                    for cand_v in candidates[v]:
                        if (cand_u, cand_v) in candidategraph.edges and (cand_u, cand_v) not in rcdgraph.edges:
                            rcdgraph.add_edge(cand_u, cand_v)
                            new_e_u.append(cand_u)
                            new_e_v.append(cand_v)
                            new_graph_v_degree[cand_u] += 1
                            new_graph_v_degree[cand_v] += 1
                            new_edge_label.append(1)
                            new_graph_v_neigh[cand_u].append(cand_v)
                            new_graph_v_neigh[cand_v].append(cand_u)
        t_2 = time.time()
        new_edges = [deepcopy(new_e_u), deepcopy(new_e_v)]
        new_vertices = new_graph_vertices
        new_v_label = new_graph_v_label
...
def train_and_test(query_vertices_num, training_percent, name_list, withouttargetstring = False):
    train_name_list = list()
    test_name_list = list()
    target_string = ''
    if not withouttargetstring:
        if query_vertices_num == '4':
            target_string = 'dense_4_'
        elif query_vertices_num == '8':
            target_string = '_8_'
        elif query_vertices_num == '9':
            target_string = '_9_'
